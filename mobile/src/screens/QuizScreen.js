...
export default function QuizScreen({ navigation }) {
  ...
  const endSession = useCallback(async () => {
    ...
    navigation.replace('Results', { ok: res.ok, error: res.error, queued: res.queued, alreadySubmitted: res.alreadySubmitted });
      ...
      <View style={styles.timerWrap}>
        {isTimed ? (
          <CircularTimer
            ...
            mode={mode}
            leftMs={globalLeftMs}
            totalMs={MODE_DURATION_S[mode] * 1000}
            label={
              mode === 'blitz'
                ? `⚡ ${t('gameStart.modes.blitz.name')}`
                : `🏃 ${t('gameStart.modes.marathon.name')}`
            }
          />
        ) : (
          <>
